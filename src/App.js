...
function App() {
  ...
  //Función para agregar nuevos proyectos a lista_datos.
  ...
  //Función para encontrar un proyecto y actualizar sus datos 
  //de acuerdo al proyecto recibido por parámetros.
  const actualizarProyecto = function(newProyect) {
    const nuevoProyectos = [...lista_datos];
    const proyect = nuevoProyectos.find( p => p.id === newProyect.id );
    for (var attr in proyect) {
      proyect[attr] = newProyect[attr];
    }
    setListaDatos(nuevoProyectos);
  }
